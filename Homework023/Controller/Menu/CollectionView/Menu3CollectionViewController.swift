...
class Menu3CollectionViewController: UICollectionViewController  {
    ...
    override func viewDidLoad() {
        ...
        MenuController.shared.fetchMenuRecords(page) { (result) in
            ...
            case .failure(let error):
                self.displayError(error, title: "Failed to Fetch Menu3 Items")
            }
        }
        configureCellSize()
    }
...
//        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath)
        ...
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as? Menu3CollectionViewCell else {return UICollectionViewCell()}
        let menuRecord = menuRecords[indexPath.item]
        cell.menu3DrinkNameLabel.text = menuRecord.fields.name
        cell.menu3DrinkMidPriceLabel.text = priceIsZeroFormate(price: menuRecord.fields.midPrice!)
        cell.menu3DrinkLargePriceLabel.text = priceIsZeroFormate(price: menuRecord.fields.largePrice!)
        ...
        MenuController.shared.fetchImage(urlString: imageUrl!) { result in
            ...
            case .success(let image):
                DispatchQueue.main.async {
                    cell.menu3DrinkImageView.image = image
                }
